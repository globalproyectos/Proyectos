...
Public Class dlgRegistroDefunciones_Reemplazos
    Public DefuncioNo As Integer
    Public FechaDf As Date
    Public LoteNo As String
    Private Sub dlgRegistroDefunciones_Reemplazos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.txtDefuncionNo.Text = DefuncioNo
        Me.txtFechaDefuncion.Text = FechaDf
        Me.txtLoteNo.Text = LoteNo
        MostrarReemplazos()
    End Sub
    Sub MostrarReemplazos()
        Try
            sql = New SqlCommand("SELECT Código_Cerda,Fecha_Nacimiento FROM REG_MADRES where Lote_No='" & LoteNo & "' and Estado_Actual<>'D' AND Estado_Actual<>'DF'", con)
            dt = New DataTable
            With dt.Columns
            ...
            Me.DataGridViewX1.Columns(3).DefaultCellStyle.BackColor = Nothing
        Catch ex As Exception
            MessageBoxEx.Show(CadenaError & ex.Message, "Error al mostrar Reemplazos", MessageBoxButtons.OK, MessageBoxIcon.Error)
            con.Close()
        End Try
    End Sub
        ...
        Try
            For i As Integer = 0 To Me.DataGridViewX1.Rows.Count - 1
                If Me.DataGridViewX1.Rows(i).Cells("Defuncion").Value = True Then
                    ...
                    sql = New SqlCommand("UPDATE REG_MADRES SET Estado_Actual='DF',Lote_No='0' WHERE Código_Cerda='" & Me.DataGridViewX1.Rows(i).Cells("Codigo").Value & "' and Lote_No='" & Me.txtLoteNo.Text & "'", con)
                    con.Open()
                    sql.ExecuteNonQuery()
                    con.Close()
                    RegistrarMovimientoLote(Me.txtLoteNo.Text, 0, 1, 0, 0, 0, 0, Me.txtFechaDefuncion.Text, "Df." & Me.txtDefuncionNo.Text)

                    sql = New SqlCommand("update Costo_Cerdas_Individual_Reemplazos set Fecha_Salio='" & Me.txtFechaDefuncion.Text & "' where Codigo_Cerda='" & Me.DataGridViewX1.Rows(i).Cells("Codigo").Value & "'", con)
                    con.Open()
                    sql.ExecuteNonQuery()
                    con.Close()
                End If
